...
transform_node = news_dynamodb_node.toDF().withColumn(
    "tags", concat_ws(",", col("tags"))
)
transform_node = transform_node.withColumn("symbols", concat_ws(",", col("symbols")))
transform_node = transform_node.withColumn(
    "neg_sentiment", coalesce(col("sentiment.neg.long"), col("sentiment.neg.double"))
)
transform_node = transform_node.withColumn(
    "pos_sentiment", coalesce(col("sentiment.pos.long"), col("sentiment.pos.double"))
)
transform_node = transform_node.withColumn(
    "new_sentiment", coalesce(col("sentiment.neu.long"), col("sentiment.neu.double"))
)
...
transform_node = transform_node.withColumn(
    "symbol", split(transform_node["symbol:link"], "#").getItem(0)
)
...
DDBDelete(
    "news", lambda x: {"date": x["date"], "symbol:link": x["symbol:link"]}
).process(news_dynamodb_node)
