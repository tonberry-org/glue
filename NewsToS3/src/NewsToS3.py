...
from pyspark.sql.functions import concat_ws, col, coalesce, to_date
...
dedup_node = DynamicFrame.fromDF(news_dynamodb_node.toDF().dropDuplicates(['link']), context, 'dedup_node')


transform_node = dedup_node.toDF().withColumn('tags', concat_ws(',', col('tags')))
